  ...
  setDialogOption: model => {
    ...
    op.forms = model.model.forms.map(item => {
      ...
      switch (item.type) {
        case 'file':
        // TODO: 完善上传
        obj.html = `el-upload(action="/base/attachment/upload" :limit="1" :on-success="upLoadSuccess" :fileList="fileList")\n  el-button(type="primary") 点击上传`
        op.datas.push(`fileList: []`)
        op.methods.push(`
/**
 * @description upLoadSuccess 上传成功回调
 * @param {null}
 */
upLoadSuccess () {
